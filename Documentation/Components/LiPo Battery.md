- Must support max current
- Motors: 10A $\times$ 4 = 40A (Limit current to 10A in ESC software)
- https://oscarliang.com/lipo-battery-guide/
# Selection
## Selected Battery
- Same but in 2 pack combo: https://chinahobbyline.com/products/cnhl-gplus-series-3000mah-22-2v-6s-70c-lipo-battery-with-xt90-plug?variant=42646821175510
- Has XT90 plug. So XT90 male to XT60 female for charger
- [Adaptor](https://www.aliexpress.com/item/1005010093372365.html): XT60 F - XT90 M 5Pcs

## Charger
- [HOTA D6 Pro](https://www.aliexpress.com/item/1005006358725398.html)
- Battery charger (first option with just DC charging will do): https://www.aliexpress.com/item/1005004413448081.html

# Parameters
## Capacity
- The capacity of a LiPo battery, measured in mAh (milliampere-hour), indicates how much current you can draw from the battery continuously for an hour until it’s empty
- 5 inch: 900mah - 1300mah

## C Rating
- C Rating is an indicator of the maximum current you can safely draw from a LiPo battery without causing damage. In theory, this can be calculated by:

```
Maximum Current Draw = Capacity x C-Rating
```
- Batteries with higher C-ratings tend to be heavier and larger even at the same capacity

## Connector
- The most common discharge connector is the XT60, mainly used in 5″ FPV drones or larger.

